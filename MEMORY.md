...
- [P1][ts:2026-02-23] LINE 貼圖進度：正在做 queue 雙模式（`QUEUE_MODE=redis|local`）以支援無 Redis 開發；目前有未提交改動於 `.env.example`、`README.md`、`scripts/dev-local.ps1`、`services/api/src/server.ts`、`services/worker/src/worker.ts`。
